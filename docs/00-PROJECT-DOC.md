...
- **Backend (FastAPI)** — single source of truth, owns RBAC, lifecycle, ETL, and the ML integration
  boundary. The separate model service is not implemented in the current backend release.
...
2. **ETL (Celery worker)**: validated telemetry → risk scoring remains an external integration boundary
...
3. **ML scoring**: deferred until the model service contract is supplied. Historical training data must
  remain separate from live telemetry regardless of implementation timing.
