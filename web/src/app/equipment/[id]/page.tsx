...
export default async function EquipmentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const workspace = readOnly ? "/executive" : "/dashboard";
  ...
  return <main className="detail-shell equipment-workspace"><header className="detail-header"><div><span className="eyebrow">EQUIPMENT / MONITORING & RECOVERY</span><h1>{id}</h1><p>{latest?.station_id ?? "Equipment station not supplied"} ? Sensor data, failure events, and accountable maintenance.</p></div><div className="detail-actions"><Link href="/equipment">All equipment</Link><Link href={workspace}>Back to workspace</Link><LiveUpdates /><LogoutButton /></div></header>
    ...
    {alertResult.available ? <EquipmentAlerts alerts={alerts} orders={orders} technicians={roster.available ? technicians : undefined} allowAssignment={!readOnly && orderResult.available && roster.available} ordersAvailable={orderResult.available} /> : <ApiNotice result={alertResult} label="Failure alerts unavailable" />}
